...
class PolicyGradientAgentInsul():
    ...
    def build_net(self):
        with tf.variable_scope('parametersi', reuse=tf.AUTO_REUSE):
        ...
        with tf.variable_scope('layer1i', reuse=tf.AUTO_REUSE):
            l1 = tf.layers.dense(inputs=self.input, units=self.layer1_size,
                                 activation=tf.nn.relu,
                      kernel_initializer=tf.contrib.layers.xavier_initializer())

        with tf.variable_scope('layer2i', reuse=tf.AUTO_REUSE):
            l2 = tf.layers.dense(inputs=l1, units=self.layer2_size,
                                 activation=tf.nn.relu,
                      kernel_initializer=tf.contrib.layers.xavier_initializer())
            
        with tf.variable_scope('layer3i', reuse=tf.AUTO_REUSE):
            l3 = tf.layers.dense(inputs=l2, units=self.layer3_size,
                                 activation=tf.nn.relu,
                      kernel_initializer=tf.contrib.layers.xavier_initializer())

        with tf.variable_scope('layer4i', reuse=tf.AUTO_REUSE):
            l4 = tf.layers.dense(inputs=l3, units=self.n_actions,
                                 activation=None,
                      kernel_initializer=tf.contrib.layers.xavier_initializer())
        self.actions = tf.nn.softmax(l4, name='actionsi')

        with tf.variable_scope('lossi', reuse=tf.AUTO_REUSE):
        ...
        with tf.variable_scope('traini', reuse=tf.AUTO_REUSE):
